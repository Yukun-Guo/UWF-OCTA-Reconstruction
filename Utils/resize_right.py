...
def resize(input, scale_factors=None, out_shape=None,
    ...
    elif interp_method == 'box':
        interp_method = box
    elif interp_method == 'lanczos2':
        interp_method = lanczos2
    elif interp_method == 'lanczos3':
...
def set_framework_dependencies(x):
    if type(x) is numpy.ndarray:
        def to_dtype(a): return a
        fw = numpy
    else:
        def to_dtype(a): return a.to(x.dtype)
        fw = torch
    eps = fw.finfo(fw.float32).eps
...
def cubic(x):
    ...
    absx2 = absx ** 2
    absx3 = absx ** 3
    return (1.5 * absx3 - 2.5 * absx2 + 1.0) * to_dtype(absx <= 1.0) + (-0.5 * absx3 + 2.5 * absx2 - 4.0 * absx + 2.0) * to_dtype((absx > 1.0) & (absx <= 2.0))
